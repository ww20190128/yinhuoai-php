...
class Brokerage extends ServiceBase
{
     ...
     * @return array
     */
    public function createShareInfo($userId)
    {
    	...
    	$now = $this->frame->now;
    	$privateKey = md5(SHARE_PRIVATE_KEY . $userEtt->parentUserId);
    	$amount = self::getShareYieldAmount($userEtt);
    	$info = array(
    		$now, // 当前时间
    		$userId, // 用户id
    		$amount, // 推广佣金
    	...
    	$shareUrlBase = $this->frame->conf['web_url'];
    
    	// 生成推广二维码   链接 & 信息 加密  (分享用户uid   分享时间， 设备信息   ip地址  网络环境， 秘钥 )
    	$url = $shareUrlBase . "/detail?&hasParams=1&shareCode=" . $encryptInfo;
    	return array(
    		'amount' => self::getShareYieldAmount($userEtt),
    		'shareUrl' => $url, // 二维码
     ...
     * @return array
     */
    public static function getShareYieldAmount($userEtt)
    {
    	$value  = 0;
    	if (empty($userEtt->parentUserId)) { // 一级推广员
    		$value = 400.00;
    	} else { // 二级推广员
    		$value = 200.00;
    	}
    ...
    public function yieldList($info, $pageNum = 1, $pageLimit = 200, $getOrder = false)
    {
    	...
    	if (is_iteratable($yieldRecordEttList)) foreach ($yieldRecordEttList as $yieldRecordEtt) {
    		$shareUserIds[] = intval($yieldRecordEtt->shareUserId);
    		$orderIds[] = intval($yieldRecordEtt->orderId);
    	}
    	...
    	if (is_iteratable($yieldRecordEttList)) foreach ($yieldRecordEttList as $yieldRecordEtt) {
    		if (empty($testOrderEttList[$yieldRecordEtt->orderId])) {
    			continue;
    		}
    		$orderEtt = $testOrderEttList[$yieldRecordEtt->orderId];
    		...
    		$list[$yieldRecordEtt->id] = array(
    			...
    			'orderInfo' => $orderEtt->getModel(),
    		);
    		if (!empty($getOrder)) {
    			$orderEtt = $orderEtt->order;
    			$orderModel = array();
