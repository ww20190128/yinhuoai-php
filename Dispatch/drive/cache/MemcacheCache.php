...
class MemcacheCache implements ICache
{
    ...
    public function __construct(array $args)
    {
        ...
        $failBack = array($this, 'failBack');
        $this->cachePrefix = $args['serverId'] . ':'; // 服务器Id
        unset($args['serverId']);
        foreach ($args as $arg) {
            list($host, $port) = array_values($arg);
            $this->client->addserver($host, $port, true, 10, 1, -1, true, $failBack);
        }
        $this->reset();
        // 运行模式
     ...
     * @param   int     $tcpPort    tcp端口号
     * @param   int     $udpport    udp端口号
     * @param   int     $error      错误消息
     * @param   int     $errno      错误码
     */
    public function failBack($host = null, $tcpPort = null, $udpport = null, $error = null, $errno = null)
    {
        $pid = posix_getpid();
        if ($errno > 0) {
            $this->client->close();
            $tries = $this->tries;
            while (--$tries > 3) {
                if ($this->client->connect($host, $tcpPort)) {
    ...
    {	         
        return $this->client->add($key, $value, false, $ttl);
    }
    ...
    private function execSet($key, $value = null, $ttl = null)
    {
        $tries = $this->tries;
        do {
            $ok = $this->client->set($key, $value, 0, $ttl);
        } while (!$ok && --$tries > 0);
    	if (!$ok) {
