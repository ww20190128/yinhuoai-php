 ...
 *  @author wangwei
 */
class PDODb implements IDaoHelper
{
    ...
    {
        if (empty($this->entityList[$table])) {
        	return 'stdClass';
        } else {
    ...
    public static function init($args = array())
    {
    	...
    	if (!empty($args)) foreach ($args as $arg) {
    		if (!empty($arg['is_main'])) {
    			self::$dbName = $arg['db_name'];
    		}
    	}
        self::$connArgs = $args;
        return;
    ...
    {
        if (empty(self::$connArgs) || empty(self::$connArgs[self::$dbName])) {
        	return false;
        }
        $connArg = self::$connArgs[self::$dbName]; // 当前数据库连接的参数
...
// print_r($sql . "\n");
     ...
     * @return object
     */
    public function fetchBySql($sql, $table = null, $param = array(), $fetchMode = self::FETCH_MODE_ALL, &$intoObj = null)
    {
     ...
     * @return array
     */
    public function fetchAll($table, $fields = '*', $where = '1', $fetchMode = self::FETCH_MODE_ALL)
    {
    ...
    {
        $where .= ' LIMIT 1;';
        return $this->fetchBySql($this->generateSelectSql($table, $fields, $where), $table, null, self::FETCH_MODE_ROW, $intoObj);
    }
     ...
     * @return object
     */
    public function fetchOne($table, $fields, $where = '1')
    {
        $where .= ' LIMIT 1;';
        return $this->fetchBySql($this->generateSelectSql($table, $fields, $where), $table, self::FETCH_MODE_ONE);
    }
     ...
     * @return array
     */
    public function fetchCol($table, $fields, $where = '1')
    {
        return $this->fetchBySql($this->generateSelectSql($table, $fields, $where), $table, self::FETCH_MODE_COL);
    }
