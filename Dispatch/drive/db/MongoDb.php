...
class MongoDb
{
	...
	private function connect() 
	{
		...
		$options = array(
			'connect'	=> true, 																	// 是否持久链接
			'timeout'	=> empty(self::$connArgs['timeout']) ? 0 : self::$connArgs['timeout'], 	// 超时时间, 默认不超时
		);
		try {
	 ...
	 */
	public function fetch($whereArr = array(), $fieldArr = array(), $limitNum = null, $sortArr = array(), $onlyCount = false)
	{
		...
		foreach ($cursor as $document) {
			$document['mongoRecordTime'] 	= $document['_id']->getTimestamp();
			$document['mongoRecordId'] 		= $document['_id']->getInc();
			$result[] = $document;
		}
		return $result;
	}
